...
[`hyprland.lua`](hyprland.lua) preserves the 10/20px gaps with 2px warm-stone borders, 10px rounding shared by Hyprland and Quickshell, the shadow/blur profile, full-opacity windows, and Space Monkey's custom sliding animations. The old `vibrancy = 80` setting was normalized to Quattro/Hyprland's current `0.0–1.0` scale as `0.8`.
